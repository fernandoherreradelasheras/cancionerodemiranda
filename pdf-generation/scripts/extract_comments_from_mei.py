...
ACCID_SYMBOL = {"s": "♯", "f": "♭", "n": "♮", "ss": "♯♯", "x": "♯♯", "ff": "♭♭"}

# Editorial container elements: a label + the description of their children.
CONTAINER_LABELS = {
    "orig": "Original:", "reg": "Regularización:",
    "corr": "Corrección:", "sic": "Sic:",
    "unclear": "Ilegible:", "supplied": "Suplido:",
    "abbr": "Abreviatura:", "expan": "Expansión:",
}

def note_accid(note):
    """(symbol, editorial) for a note: the accidental glyph to append to the
    pitch name, and whether it is an editorial accidental. The accidental is on
    the note's @accid, or on a child <accid> (whose @func="edit" marks it as
    editorial)."""
    if note.get("accid"):
        return ACCID_SYMBOL.get(note.get("accid"), ""), False
    child = note.find("mei:accid", NSMAP)
    if child is not None and child.get("accid"):
        return ACCID_SYMBOL.get(child.get("accid"), ""), child.get("func") == "edit"
    return "", False

def describe(element):
    if element.tag == f'{{{MEI_NS}}}note':
        symbol, editorial = note_accid(element)
        editorial_note = " (alteración editorial)" if editorial else ""
        return (f' {element.attrib["pname"]}{symbol}{element.attrib["oct"]} '
                f'({element.attrib["dur"]}){dots(element)}{editorial_note}')
    ...
    elif element.tag.split('}')[-1] in CONTAINER_LABELS:
        text = CONTAINER_LABELS[element.tag.split('}')[-1]]
        for e in element.xpath('./mei:*', namespaces=NSMAP):
            text = text + (describe(e) or "")
        return text
...
if output_file:
    ...
    if editorial_notes is not None and len(editorial_notes) > 0:
        ...
        for note in editorial_notes:
            # Collapse insignificant XML whitespace so escape_latex doesn't emit \newline.
            text = escape_latex(" ".join((note['annotText'] or "").split()))
